...
class PyVmomiHelper(object):
    ...
    def deploy_template(self, name=None, poweron=False, wait_for_ip=False):
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            exact_count=dict(required=True, type='int'),
            exact_count_padding=dict(required=False, type='int', default=2),
    ...
    # Generate the list of expected names
    vm_prefix = module.params['name']
    vm_count = module.params['exact_count']
    vm_count_padding = module.params['exact_count_padding']
    expected_vms = []
    for x in range(0, vm_count):
        x = str(x)
        if len(x) < vm_count_padding:
            total_zeros = vm_count_padding - len(x)
            zeros = ''
            for z in range(0, total_zeros):
                zeros += '0'
            x = zeros + x
        expected_vms.append(x)
    expected_vms = [vm_prefix + '_' + x for x in expected_vms]

    # Generate the list of undesired names
    unexpected_vms = []
    for x in range(0, 50):
        x = str(x)
        if len(x) < vm_count_padding:
            ...
            for z in range(0, total_zeros):
                zeros += '0'
            x = zeros + x
        unexpected_vms.append(x)
    unexpected_vms = [vm_prefix + '_' + x for x in unexpected_vms]
    unexpected_vms = [x for x in unexpected_vms if x not in expected_vms]

    result = {}

    # Delete any undesired VMs
    for uvm in unexpected_vms:
        vm = pyv.getvm(name=uvm, 
    ...
    # Create any desired VMs
    for evm in expected_vms:

        this_result = {}

        # Check if the VM exists before continuing
        vm = pyv.getvm(name=evm, 
                       folder=module.params['folder'], 
                       name_match=module.params['name_match'])
        ...
        if vm:
            ...
            if module.params['state'] == 'absent':
                ...
                if module.params['force']:
                    ...
                    this_result = pyv.set_powerstate(vm, 'poweredoff', module.params['force'])
                this_result = pyv.remove_vm(vm)
        ...
        # VM doesn't exist
            ...
            if module.params['state'] in create_states:
                ...
                # Create it ...
                this_result = pyv.deploy_template(
                            name=evm,
                            poweron=poweron, 
                            wait_for_ip=False
                         )
            elif module.params['state'] == 'absent':
                this_result = {'changed': False, 'failed': False}
            else:
                this_result = {'changed': False, 'failed': False}

        result[evm] = this_result
    ...
    if module.params['wait_for_ip_address']:
        for evm in expected_vms:
            ...
            while not vm:
                ...
                if not vm:
                    time.sleep(2)
            result[evm] = pyv.set_powerstate(vm, 'poweredon', force=False)
